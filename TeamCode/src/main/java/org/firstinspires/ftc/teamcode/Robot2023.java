...
public class Robot2023 extends Robot {
    ...
    public void teleOp() {
        ...
        UP.setPower(((gamepad2.right_stick_y)*(gamepad2.right_stick_y)*Math.signum(gamepad2.right_stick_y))*0.5);
    ...
    public  void go(double cm) { //
        ...
        while ( ( Math.abs(D) > 0.1 ||  Math.abs(cc) - Math.abs(LF.getCurrentPosition()) > 10*Math.signum(cc)) && linearOpMode.opModeIsActive()) {
            ...
            double Er2 = Math.abs(cc) - Math.abs(RB.getCurrentPosition());
            ...
            double pwf2 =  (pw * (P2+Rele2)*Math.signum(Er2));
            double pwf = (pw * (P+Rele))*Math.signum(Er); //Регулятор
            double pwfinal = (pwf + pwf2)/2;
